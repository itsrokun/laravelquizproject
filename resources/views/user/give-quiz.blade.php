    ...
    <div class="text-center">
        <form method="post" action="{{route('store.answer')}}">
            ...
            @foreach($questions as $question)
                <div>
                    <label><strong>Question {{$i++}}:</strong> {{$question->question}}</label>

                    <!-- Display the diagram if available -->
                    @if($question->question_diagram)
                        <div>
                            <label><strong>Question Diagram:</strong></label>
                            <img src="{{ asset('storage/' . $question->question_diagram) }}" alt="Question Diagram" style="max-width: 100%; height: auto;">
                            <br>
                            <span><strong>File Path:</strong> {{ asset('storage/' . $question->question_diagram) }}</span>
                        </div>
                        <hr>
                    @endif
                    ...
                    <select name="answer[{{$i}}]" class="form-control" required>
                        <option selected disabled value>Choose an option for this question</option>
